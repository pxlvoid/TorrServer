...
- Порядок наводит уборщик (раз в минуту и сразу при старте): держит весь кэш в пределах бюджета, выбрасывая
  ...
  **N дней**. То, что сейчас смотрят, и **закреплённые** (★) торренты уборщик не трогает.
...
- Диалог «Кэш на диске»: занято/лимит/свободно, настройки, карточки закэшированного (постер и короткое название из
  ...
  удалить, «Очистить незакреплённое». Карточки компактные: действия (⬇/■, ★, 🗑) — строкой у названия, под ним полоса,
