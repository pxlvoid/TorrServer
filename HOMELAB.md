...
- Диалог «Кэш на диске»: занято/лимит/свободно, настройки, карточки закэшированного (постер и короткое название из
  списка TorrServer, доля торрента на диске, когда смотрели), закрепить, удалить, «Очистить незакреплённое».
...
| `web/src/components/DialogTorrentDetailsContent/index.jsx` | 2 | в постоянном режиме вместо мини-«змейки» — полоса торрента: что на диске, где плеер (сама змейка — внутри обёртки) |
