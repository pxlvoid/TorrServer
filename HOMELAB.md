...
- **Кто смотрит** — каждый запрос плеера к `Torrent.Stream` (Лампа, VLC, «Открыть») учитывается в той же обёртке
  ...
  15 с. Показывается: сводка над списком торрентов («▶ Смотрят сейчас: 2 · 36 Мбит/с» и строка на устройство), бейдж
  «▶ 18 Мбит/с» на постере, список в окне торрента. Не учитываются HLS-плеер GStreamer, DLNA, WebDAV/FUSE и запросы
