...
- **Кто смотрит** — каждый запрос плеера к `Torrent.Stream` (Лампа, VLC, «Открыть») учитывается в той же обёртке
  ...
  15 с. Показывается: отдельная карточка «Смотрят сейчас» над списком торрентов (под сводкой кэша, только пока кто-то
  смотрит: устройство, что, позиция, скорость) — клик открывает окно с подробностями (плеер и адрес, файл, позиция в
  ГБ, скорость сейчас / в среднем и её график за последние минуты, отдано, с какого времени, сколько файла на диске и
  что торрент качает из сети, пиры); бейдж «▶ 18 Мбит/с» на постере; список в окне торрента. Не учитываются HLS-плеер GStreamer, DLNA, WebDAV/FUSE и запросы
